...
X_train, X_test, y_tmin_train, y_tmin_test, y_tmax_train, y_tmax_test = train_test_split(X, y_tmin, y_tmax, test_size=0.2, random_state=142)
...
plt.plot(df['date'], df['tmax'], label='Actual TMax')
plt.plot(future['date'], tmin_forecast, label='Predicted TMin')
plt.plot(future['date'], tmax_forecast, label='Predicted TMax')
plt.legend()
plt.xlabel('Date')
plt.ylabel('TMin')
...
tmin_error = 100 -(mean_absolute_error(y_tmin_test, tmin_test_forecast) / y_tmin_test.mean() * 100)
tmax_error = 100 -(mean_absolute_error(y_tmax_test, tmax_test_forecast) / y_tmax_test.mean() * 100)
print (tmax_error)
print (tmin_error)

plt.figure(figsize=(10, 6))
plt.plot(df['date'], (df['tmin'] + df['tmax']) /2, label='Actual TM')
plt.plot(future['date'], (tmin_forecast+tmax_forecast)/2 , label='Predicted TM')
plt.legend()
plt.xlabel('Date')
plt.ylabel('TM')
plt.title('Actual vs. Predicted TMin')
...
axs[0].plot(df['date'], df['tmin'], label='Actual TMin')
axs[0].plot(future['date'], tmin_forecast, label='Predicted TMin')
axs[1].plot(df['date'], df['tmax'], label='Actual TMax')
axs[1].plot(future['date'], tmax_forecast, label='Predicted TMax')
plt.show()